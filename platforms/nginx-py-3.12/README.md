...
COMPOSE_PROJECT_IMGK="alpine-3.24-nginx-py-3.12"        # <- real main image keys to manage automations for sharing resources -------------------------------> #
...
`./platform/nginx-py-3.12/docker/config/supervisor/conf.d-sample`

Before building the container you must copy that directory to:
`./platform/nginx-py-3.12/docker/config/supervisor/conf.d`
...
```bash
$ cd ./platform/nginx-py-3.12/docker/config/supervisor
